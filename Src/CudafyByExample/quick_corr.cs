...
namespace CudafyByExample
{
    class quick_corr
    {
        ...
        [Cudafy]
        public static void QuickCorr(GThread thread, ProfileStrucuture[, ,] profile_GPU, ForeGroundStrucuture[] foregorungRGB_GPU, BackGroundStrucuture[] BackgroundXYZ_GPU, double[] ptr, SampleStructure[,] samples)
        {
        ...
        public static void Execute()
        {
            ...
            try
            {
                for (int i = 1; i < profile.Rows.Count; i++)
                {
                    ...
                    //if (lvalue == 20 && avalue == 20 && bvalue == 22)
                    //    Console.WriteLine("{0},{1},{2} - {3},{4},{5}", temp_l, temp_a, temp_b, lvalue, avalue, bvalue);
            ...
            for (int i = 0; i < 22; i++)
            {
                ...
                //quick_correct
                gpu.Launch(grids, threads, ((Action<GThread, ProfileStrucuture[, ,], ForeGroundStrucuture[], BackGroundStrucuture[], double[], SampleStructure[,]>)QuickCorr), profile_GPU, foregorungRGB_GPU, BackgroundXYZ_GPU, distance_GPU, samples_GPU);
